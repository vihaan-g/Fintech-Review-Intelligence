...
def test_run_excludes_chairman_from_stage1_and_runs_stage2b_specialists(llm_env) -> None:
    ...
    with DatabaseManager(db_path=":memory:") as db:
        ...
        for member in orchestrator.specialist_members:
            async def specialist_generate(prompt: str, member=member) -> MemberResponse:
                ...
                return _response(member.name, member.model_id, f"stage1 {member.name}")

            member.generate = specialist_generate  # type: ignore[method-assign]
        ...
        async def chairman_generate(prompt: str) -> MemberResponse:
            ...
            if prompt_lower.endswith("analytical frame:"):
                return _response(orchestrator.chairman.name, orchestrator.chairman.model_id, "frame")
        ...
        orchestrator.chairman.generate = chairman_generate  # type: ignore[method-assign]
    ...
    with DatabaseManager(db_path=":memory:") as db:
        ...
        for member in orchestrator.specialist_members:
            async def specialist_generate(prompt: str, member=member) -> MemberResponse:
                ...
                return _response(member.name, member.model_id, "stage1 insight")
